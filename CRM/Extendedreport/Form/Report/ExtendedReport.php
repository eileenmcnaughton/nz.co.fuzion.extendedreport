...
class CRM_Extendedreport_Form_Report_ExtendedReport extends CRM_Report_Form {
   ...
   * @throws \CRM_Core_Exception
   */
  protected function aggregateSelect() {
   ...
   * @throws \CRM_Core_Exception
   */
  protected function addColumnAggregateSelect(string $fieldName, $dbAlias, $spec) {
    if (empty($fieldName)) {
   ...
   * @return string
   */
  function getFilterFieldValue(&$spec): string {
   ...
   * @throws \CiviCRM_API3_Exception
   */
  protected function _getOptions($entity, $field, $action = 'get'): array {
    static $allOptions = [];
    $key = "{$entity}_{$field}";
   ...
   *
   */
  protected function reOrderColumnHeaders() {
    $fieldMap = [];
    foreach ($this->_columns as $tableName => $table) {
   ...
   * @throws \CiviCRM_API3_Exception
   */
  public function buildContributionTotalAmountBybreakdown($rowFieldId, string $columnType, string $header): ?array {
   ...
   * @return string
   */
  protected function selectStatSum(&$tableName, &$fieldName, &$field): string {
   ...
   * @throws \CRM_Core_Exception
   */
  function buildQuery($applyLimit = TRUE): string {
...
LEFT JOIN civicrm_contact {$prop['alias']} ON {$prop['alias']}.id = {$this->_aliases[$tableKey]}.{$prop['column_name']} ";
   ...
   * @throws \Exception
   */
  protected function extractCustomFields(&$customFields, $context = 'select'): array {
    $myColumns = [];
...
WHERE cg.extends IN ('" . implode("','", $extends) . "') AND
   ...
   * @param $rows
   */
  protected function assignSubTotalLines(&$rows): void {
    foreach ($rows as $index => & $row) {
   ...
   * @throws \CiviCRM_API3_Exception
   */
  protected function getLineItemColumns($options): array {
    $defaultOptions = [
      'prefix' => '',
      'prefix_label' => '',
      'group_by' => TRUE,
   ...
   * @throws \CiviCRM_API3_Exception
   */
  protected function getPriceFieldValueColumns($options): array {
   ...
   * @throws \CiviCRM_API3_Exception
   */
  protected function getMembershipColumns($options): array {
    $specs = [
      'membership_type_id' => [
   ...
   * @throws \CiviCRM_API3_Exception
   */
  protected function getMembershipLogColumns($options = []): array {
    $columns = [
   ...
   * @throws \CiviCRM_API3_Exception
   */
  protected function getFinancialAccountColumns($options = []): array {
    $defaultOptions = [
      'prefix' => '',
      'prefix_label' => '',
      'group_by' => TRUE,
   ...
   * @throws \CiviCRM_API3_Exception
   */
  protected function getPledgePaymentColumns($options): array {
    $specs = [
   ...
   * @throws \CiviCRM_API3_Exception
   */
  protected function getEventColumns($options = []): array {
    $specs = [
      'event_id' => [
        'name' => 'id',
        'is_fields' => TRUE,
   ...
   * @throws \CiviCRM_API3_Exception
   */
  protected function getEmailColumns($options = []): array {
    $defaultOptions = [
      'prefix' => '',
      'prefix_label' => '',
      'group_by' => FALSE,
   ...
   * @throws \CiviCRM_API3_Exception
   */
  protected function getWebsiteColumns($options = []): array {
    $defaultOptions = [
      'prefix' => '',
      'prefix_label' => '',
      'group_by' => FALSE,
   ...
   * @throws \CiviCRM_API3_Exception
   */
  protected function getNoteColumns($options = []): array {
    $defaultOptions = [
      'prefix' => '',
      'prefix_label' => '',
      'group_by' => FALSE,
  ...
  */
   ...
   * @return bool true or false to denote whether extra filters can be appended to join
   */
  protected function joinAddressFromContribution($prefix = 'billingaddress', $extra = []): bool {
...
{$this->_aliases['civicrm_event']}.is_template = 0)";
   ...
   */
  function getCriteriaString(): string {
   ...
   * @return string
   */
  protected function alterCrmEditable($value, &$row, $selectedField, $criteriaFieldName, $fullSpec): string {
   ...
   * @return string
   */
  protected function alterFinancialType($value, &$row, $selectedField, $criteriaFieldName, $specs): string {
...
*/
   ...
   * @throws \CiviCRM_API3_Exception
   */
  public function alterLocationTypeID($value, &$row, $selectedfield, $criteriaFieldName) {
   ...
   * @throws \CRM_Core_Exception
   */
  protected function addFilterFieldsToReport($field, $fieldName, $table, $count, $prefix): void {
